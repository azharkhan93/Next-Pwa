...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className={`relative ${className ?? ""}`} ref={dropdownRef}>
      {label && (
        <label htmlFor={id || name} className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">
          {label}
        </label>
      )}
      <button
        ...
        disabled={disabled}
        className="w-full px-3 py-1.5 text-sm flex items-center justify-between border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
